/* ─────────────────────────────────────────────────────────────────────────────
 * ROLLBACK SNAPSHOT — paste the CURRENT live Webflow component below this header,
 * before overwriting it with dist/nem-test-phase-b.webflow.tsx.
 *
 * Why this file exists: the Webflow component is edited in place and may have
 * drifted from the repo. If it has, git history is not a rollback point for it —
 * this file is the only one.
 *
 * To restore: copy everything below the header back into the Webflow component.
 *
 * Captured: 2026-08-17, before the conclusion engine v2 paste.
 * ───────────────────────────────────────────────────────────────────────────── */

// ⬇⬇⬇ PASTE THE LIVE WEBFLOW COMPONENT BELOW THIS LINE ⬇⬇⬇
...
function calculateScores(answers: (number | null)[]) {
  ...
  const conclusionKey = secondary ? `${primaryKey}_${MECHANISM_TO_KEY[secondary]}` : primaryKey;
...
const NL_CONCLUSIONS_SINGLE: Record<string, string> = {
  ...
  'zelfafwijzing_emotionele-verdoving':
    '[DUMMY man] Zelfafwijzing + Emotionele Verdoving — Alex schrijft deze tekst nog.',
  'zelfafwijzing_valse-macht':
    '[DUMMY man] Zelfafwijzing + Valse Macht — Alex schrijft deze tekst nog.',
  zelfafwijzing_angst: '[DUMMY man] Zelfafwijzing + Angst — Alex schrijft deze tekst nog.',
  'zelfafwijzing_valse-hoop':
    '[DUMMY man] Zelfafwijzing + Valse Hoop — Alex schrijft deze tekst nog.',
  'emotionele-verdoving_valse-macht':
    '[DUMMY man] Emotionele Verdoving + Valse Macht — Alex schrijft deze tekst nog.',
  'emotionele-verdoving_angst':
    '[DUMMY man] Emotionele Verdoving + Angst — Alex schrijft deze tekst nog.',
  'emotionele-verdoving_valse-hoop':
    '[DUMMY man] Emotionele Verdoving + Valse Hoop — Alex schrijft deze tekst nog.',
...
const NL_CONCLUSIONS_VROUW: Record<string, string> = Object.fromEntries(
  Object.entries(NL_CONCLUSIONS_SINGLE).map(([k, v]) => [
    k,
    v.replace('[DUMMY man]', '[DUMMY vrouw]'),
  ]),
);
...
const EN_CONCLUSIONS_MAN: Record<string, string> = {
  ...
  'zelfafwijzing_emotionele-verdoving':
    '[DUMMY man] Self-rejection + Emotional Numbing — Alex will write this.',
  ...
  'emotionele-verdoving_valse-macht':
    '[DUMMY man] Emotional Numbing + False Power — Alex will write this.',
  'emotionele-verdoving_angst': '[DUMMY man] Emotional Numbing + Fear — Alex will write this.',
  'emotionele-verdoving_valse-hoop':
    '[DUMMY man] Emotional Numbing + False Hope — Alex will write this.',
...
const EN_CONCLUSIONS_VROUW: Record<string, string> = Object.fromEntries(
  Object.entries(EN_CONCLUSIONS_MAN).map(([k, v]) => [
    k,
    v.replace('[DUMMY man]', '[DUMMY vrouw]'),
  ]),
);
...
const translations: Record<'nl' | 'en', Translations> = {
  nl: {
    ...
    genderLabel: 'Je geslacht',
    genderOptions: [
      { value: '', label: 'Maak een keuze..' },
      { value: 'man', label: 'Man' },
      { value: 'vrouw', label: 'Vrouw' },
    ],
    ageCategoryLabel: 'Je leeftijdscategorie',
    ageCategoryOptions: [
      { value: '', label: 'Maak een keuze..' },
      { value: '18-30', label: '18-30 jaar' },
  ...
  en: {
    ...
    optinIntro: "Fill in your details below. You'll receive it in your inbox within minutes.",
    firstNamePlaceholder: 'Your first name',
    emailPlaceholder: 'Your email address',
    relationshipLabel: 'Your relationship status',
    relationshipOptions: [
      { value: '', label: 'Select...' },
      { value: 'single-without-children', label: 'Single - without children' },
      { value: 'single-with-children', label: 'Single - with children' },
      { value: 'with-a-partner-without-children', label: 'With a partner - without children' },
      { value: 'with-a-partner-with-children', label: 'With a partner - with children' },
    ...
    ageCategoryLabel: 'Your age category',
    ageCategoryOptions: [
      { value: '', label: 'Select...' },
      { value: '18-30', label: '18-30 year' },
      { value: '31-40', label: '31-40 year' },
      { value: '41-50', label: '41-50 year' },
      { value: '51-60', label: '51-60 year' },
      { value: '60+', label: '60+ year' },
    ],
    consentLabel:
...
function Quiz({
  ...
  const conclusionText =
    t.conclusions[genderKey as keyof GenderedConclusions]?.[conclusionKey] || '';
  ...
  return (
    ...
    >
      ...
      ═══════════════════════════════════════════ */}
      {phase === 'quiz' && (
        <div key={currentStep} className={animating ? 'quiz-fade-out' : 'quiz-fade-in'}>
          {/* Step indicator */}
          <div
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    backgroundColor: isSelected ? 'var(--_token---accent-main, #fafa7d)' : 'white',
                    color: 'var(--_token---text-main, #292828)',
                    cursor: 'pointer',
                    fontFamily: "'Lato', sans-serif",
                    fontSize: 'var(--_typography---paragraph--standard, 1rem)',
          ...
          <div className="flex flex-col gap-4">
            ...
            <div>
              ...
              {fieldErrors.gender && (
                <div
                  id="error-gender"
                  style={{ color: '#e53e3e', fontSize: '0.875rem', marginTop: 4 }}
                >
                  {fieldErrors.gender}
                </div>
              )}
            ...
            <div>
              ...
              <select
                ...
                onBlur={() => {
                  if (!ageCategory)
                    setFieldErrors((p) => ({ ...p, ageCategory: t.errors.ageCategoryEmpty }));
                }}
                onFocus={(e) => {
                  e.currentTarget.style.boxShadow = focusRing;
                }}
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  if (fieldErrors.relationshipStatus)
                    setFieldErrors((p) => ({ ...p, relationshipStatus: '' }));
                }}
                onBlur={() => {
                  if (!relationshipStatus)
                    setFieldErrors((p) => ({
                      ...p,
                      relationshipStatus: t.errors.relationshipEmpty,
                    }));
                }}
                onFocus={(e) => {
                  e.currentTarget.style.boxShadow = focusRing;
          ...
          <div className="flex flex-col gap-4">
            ...
            <div>
              <input
                ...
                onBlur={() => {
                  if (!firstName.trim())
                    setFieldErrors((p) => ({ ...p, firstName: t.errors.firstNameEmpty }));
                }}
                onFocus={(e) => {
                  e.currentTarget.style.boxShadow = focusRing;
                }}
            ...
            <div>
              ...
              >
                ...
                />
                <span
                  style={{ fontSize: '0.875rem', color: 'var(--_token---text-olive, #706d56)' }}
                >
                  {t.consentLabel}
                </span>
              </label>
      ...
      {phase === 'confirmation' && (
        <div
          className="quiz-scale-in flex flex-col gap-5"
...
export default declareComponent(Quiz, {
  ...
  props: {
    ...
    question1: propTypes.Text({
      name: 'Question 1',
      defaultValue:
        'Na een gesprek dat niet lekker liep, blijf ik uren of dagen malen over wat ik fout deed.',
    }),
    question2: propTypes.Text({
      name: 'Question 2',
      defaultValue:
        'Als iets in een relatie of op werk misgaat, ben ik de eerste die denkt dat het aan mij ligt.',
    }),
    question3: propTypes.Text({
      name: 'Question 3',
      defaultValue:
        'Ik stel beslissingen of dingen die ik eigenlijk wil doen langer uit dan logisch is.',
    }),
    question4: propTypes.Text({
      name: 'Question 4',
      defaultValue: 'Ik betrap mezelf erop dat ik streng oordeel over hoe anderen dingen doen.',
    }),
    question5: propTypes.Text({
      name: 'Question 5',
      defaultValue:
        'Ik vermijd situaties die spanning oproepen, ook als ik eigenlijk wel zou willen.',
    }),
    question6: propTypes.Text({
      name: 'Question 6',
      defaultValue:
        'Ik blijf mijn best doen om iets of iemand naar mijn hand te zetten, ook als ik weet dat het niet gaat lukken.',
    }),
    question7: propTypes.Text({
      name: 'Question 7',
      defaultValue: 'Als ik moe of overweldigd ben, zegt iets in mij: "Ik kan dit niet aan."',
    }),
    question8: propTypes.Text({
      name: 'Question 8',
      defaultValue:
        'Mijn standaardreactie als er iets gebeurt is: "Maakt niet uit, het komt wel goed."',
    }),
    question9: propTypes.Text({
      name: 'Question 9',
      defaultValue:
        'Als iemand iets doet wat me raakt, ervaar ik dat al snel als een persoonlijke aanval en ga ik er fel tegenin.',
    }),
    question10: propTypes.Text({
      name: 'Question 10',
      defaultValue:
        'Bij iets nieuws of onbekends ga ik in mijn hoofd direct naar wat er mis zou kunnen gaan.',
    }),
    question11: propTypes.Text({
      name: 'Question 11',
      defaultValue:
        'Ik voel een drive in mijn lichaam - gejaagd, hoog, ik kan niet stil zitten als er nog iets opgelost moet worden.',
    }),
    question12: propTypes.Text({
      name: 'Question 12',
      defaultValue: 'Ik denk vaak: "Als ik dit nou maar goed doe, dan komt het wel goed."',
    }),
    question13: propTypes.Text({
      name: 'Question 13',
      defaultValue:
        'In situaties die me eigenlijk zouden moeten raken, merk ik nauwelijks iets op in mijn lichaam - alsof ik op afstand sta van mijn eigen leven.',
    }),
    question14: propTypes.Text({
      name: 'Question 14',
      defaultValue:
        'Op het moment dat ik me aangevallen voel, voel ik mijn lichaam aanspannen - mijn kaken, mijn schouders, mijn vuisten, alsof ik me acuut wil verdedigen.',
    }),
    question15: propTypes.Text({
      name: 'Question 15',
      defaultValue:
        'In spannende situaties voel ik mijn lichaam terugdeinzen - een verkramping, het gevoel dat ik ergens van weg wil.',
    }),
    question16: propTypes.Text({
      name: 'Question 16',
      defaultValue:
        'Als de sfeer dreigt om te slaan, doe ik extra mijn best en pas ik me aan om het goed te houden.',
    }),
    question17: propTypes.Text({
      name: 'Question 17',
      defaultValue:
        'Als iets misgaat voel ik mezelf wegzakken - mijn energie verdwijnt en alles wordt zwaar.',
    }),
    question18: propTypes.Text({
      name: 'Question 18',
      defaultValue:
        'Als anderen om mij heen sterk reageren op een emotionele gebeurtenis, blijf ik vanbinnen vaak vlak.',
    }),
    question19: propTypes.Text({
      name: 'Question 19',
      defaultValue:
        'Als iemand dichtbij iets niet doet zoals ik wil, voel ik irritatie of boosheid die maar niet weggaat.',
    }),
    question20: propTypes.Text({
      name: 'Question 20',
      defaultValue:
        'Als ik iets moet doen wat goed voor me is maar me angst geeft, kies ik vaak voor wat veilig voelt.',
    }),
