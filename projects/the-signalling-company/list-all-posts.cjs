...
console.log('\n=== ALL ROWS (excluding bbsm_scene) ===');

for (const row of allRows) {
  const type = row[20];
  if (type === 'bbsm_scene') continue;
  const id = row[0];
  const date = row[2];
  ...
  console.log(`${type} | ID:${id} | ${date} | [${status}] ${title.substring(0, 80)}`);
  if (contentLen > 100) {
    console.log(`  content: ${contentLen} chars, slug: ${slug}, parent: ${parent}`);
  }
}
