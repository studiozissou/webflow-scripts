...
const submit = JSON.parse(readFileSync(path.join(BACKEND, "nem-submit.workflow.json"), "utf8"));

const node = (wf, name) => wf.nodes.find((n) => n.name === name);
const branch = (wf, name, i) => ((wf.connections[name]?.main ?? [])[i] ?? []).map((c) => c.node);
...
const profile = (overrides = {}) => ({
  ...
  scoresJson: '{"falseHope":14,"falsePower":11,"selfRejection":9,"fear":7,"emotionalNumbing":3}',
  ...
  conclusionText: "Je blijft hopen, ook als de feiten iets anders zeggen.\n\nDat kost je.",
  ...overrides,
});
...
describe("Generate Report — the user message carries what the prompt reads (7a, 7b)", () => {
  test("sends the intro line under an `Intro line:` label", () => {
    ...
    assert.ok(msg.includes("Intro line: Je hoopt dat het vanzelf goed komt & wacht af."), msg);
  });

  test("sends the conclusion text under a `Conclusion text:` label, paragraph breaks intact", () => {
    const msg = userMessage(profile());
    assert.ok(
      msg.includes("Conclusion text: Je blijft hopen, ook als de feiten iets anders zeggen.\n\nDat kost je."),
      msg,
    );
  });
...
describe("Build HTML — the prompt owns the first name (7f)", () => {
  ...
  test("no `Beste …,` line — the name appears once, inside opening", () => {
    ...
    assert.equal(out.split("Sjoerd").length - 1, 1, "first name must appear exactly once");
  });
  ...
  test("without an intro line the <h1> flows straight into opening", () => {
    ...
    assert.match(out, new RegExp("</h1><p>" + report.opening.replace(/[.*+?^${}()|[\]\\]/g, "\\$&") + "</p>"));
  });
});
...
describe("the locale gate (7d) — `en` never reaches Anthropic", () => {
  ...
  test("Unsupported Locale emits the exact record Log Failure and Alert Failure map", () => {
    ...
    assert.ok(!Number.isNaN(Date.parse(row.failedAt)), "failedAt must be an ISO timestamp");
...
describe("/submit — conclusionText rides the payload into the profile row (7b)", () => {
  ...
  test("Normalize keeps conclusionText as a string", () => {
    const out = runNormalize({ token: "t", conclusionText: "Je blijft hopen.\n\nDat kost je." });
...
describe("the changeset files cannot drift from the snapshots", () => {
  ...
  test("unsupported-locale.jsCode.js is byte-identical to the committed node", () => {
    assert.equal(read("unsupported-locale.jsCode.js"), node(verify, "Unsupported Locale").parameters.jsCode + "\n");
  });
  ...
  test("store-profile.columns.json carries the mapping and schema entries", () => {
    ...
    assert.deepEqual(file.schema[0], live.schema.find((s) => s.id === "conclusionText"));
  });
  ...
  test("the partial-update payload targets the live /verify workflow and adds both nodes", () => {
    ...
    const added = ops.operations.filter((o) => o.type === "addNode").map((o) => o.node.name);
    assert.deepEqual(added.sort(), ["Locale Supported?", "Unsupported Locale"]);
    const gr = ops.operations.find((o) => o.type === "updateNode" && o.nodeName === "Generate Report");
    assert.equal(gr.updates["parameters.jsonBody"], generateReportBody);
    const bh = ops.operations.find((o) => o.type === "updateNode" && o.nodeName === "Build HTML");
